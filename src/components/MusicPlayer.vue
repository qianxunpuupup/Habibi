    ...
    <div class="music-player" :class="{ 'is-playing': isPlaying }">
                ...
                <svg viewBox="0 0 200 200" class="cover-svg">
                  <defs>
                    <linearGradient id="coverGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                      ...
                      <stop offset="35%" style="stop-color:#FFA8C5;stop-opacity:1" />
                  ...
                  <!-- 海面 -->
                  <path d="M0 120 Q50 110 100 120 T200 120 L200 200 L0 200 Z" fill="rgba(0,105,148,0.4)"/>
                  <path d="M0 135 Q50 128 100 135 T200 135" stroke="rgba(255,255,255,0.4)" stroke-width="2" fill="none"/>
                  <path d="M0 150 Q50 143 100 150 T200 150" stroke="rgba(255,255,255,0.3)" stroke-width="1.5" fill="none"/>
                  <!-- 太阳 -->
                  <circle cx="140" cy="65" r="28" fill="url(#sunGrad)" opacity="0.95"/>
                  <!-- S路 -->
                  <path d="M0 200 C40 195 60 175 80 160 C100 145 90 125 110 110 C130 95 160 90 180 70 L200 50 L200 0 L0 0 Z" fill="rgba(139,111,71,0.3)"/>
                  <!-- 房子 -->
                  <g transform="translate(35,140)">
                    <rect x="0" y="10" width="25" height="22" fill="#FFFAF0" stroke="#CD5C5C" stroke-width="1"/>
                  ...
                  <!-- 海鸥 -->
                  <path d="M50 45 Q55 40 60 45 Q65 40 70 45" stroke="#5C4033" stroke-width="1.5" fill="none"/>
                  <path d="M75 30 Q80 25 85 30 Q90 25 95 30" stroke="#5C4033" stroke-width="1.2" fill="none"/>
                </svg>
              </div>
            </div>
            <!-- 中心孔 -->
      ...
      <div class="controls">
        <button class="ctrl-btn btn-mode" @click="toggleMode" :title="modeText">
          <svg v-if="playMode === 'loop'" viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
            <path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z"/>
          </svg>
          <svg v-else-if="playMode === 'single'" viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
        ...
        <button class="ctrl-btn btn-play" @click="togglePlay" :title="isPlaying ? '暂停' : '播放'">
        ...
        <button class="ctrl-btn btn-list" @click="showList = !showList" :class="{ active: showList }" title="播放列表">
      ...
      <div class="volume-wrap">
        <button class="vol-btn" @click="toggleMute" :title="isMuted ? '取消静音' : '静音'">
          ...
          <svg v-else-if="volume < 0.5" viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
        ...
        <div class="vol-slider" @click="setVolume">
          <div class="vol-track">
            ...
            <div class="vol-thumb" :style="{ left: (isMuted ? 0 : volume * 100) + '%' }"></div>
          </div>
        </div>
      </div>
      ...
      <div v-if="isPlaying" class="visualizer">
        <i v-for="n in 28" :key="n" class="viz-bar" :style="{ animationDelay: (n * 0.07) + 's', height: getBarHeight(n) + '%' }"></i>
      </div>
    </div>
  </div>
...
const playNote = (freq, dur = 0.3) => {
  ...
  try {
    ...
    gain.gain.exponentialRampToValueAtTime(volume.value * 0.35, audioCtx.currentTime + 0.02)
...
const melody = [
  // 频率(Hz), 时长系数
  [523.25, 1], [587.33, 1], [659.25, 1.5], [587.33, 0.5],
  [523.25, 1], [659.25, 1], [783.99, 2],
  [698.46, 1], [659.25, 1], [587.33, 1.5], [523.25, 0.5],
  [587.33, 1], [523.25, 1], [440.00, 2]
]
let noteIdx = 0
let noteTimeLeft = 0
...
const handleEnded = () => {
  ...
  if (playMode.value === 'random') {
    let idx
    do { idx = Math.floor(Math.random() * songs.length) } while (songs.length > 1 && idx === currentIndex.value)
    playSong(idx)
    return
  }
  // loop
...
const prevSong = () => {
  const idx = currentIndex.value <= 0 ? songs.length - 1 : currentIndex.value - 1
  playSong(idx)
}
const nextSong = () => {
  const idx = currentIndex.value >= songs.length - 1 ? 0 : currentIndex.value + 1
  playSong(idx)
}

const toggleMode = () => {
  playMode.value = ({ loop: 'single', single: 'random', random: 'loop' })[playMode.value]
}

const seekTo = (e) => {
  const rect = e.currentTarget.querySelector('.progress-bg').getBoundingClientRect()
...
.player-container:not(.player-open) {
  ...
  & .mini-toggle { transform: translateX(-70px) rotate(180deg); }
  & .mini-toggle.flipped { transform: translateX(-70px) rotate(0deg); }
}
...
.cover-outer {
  ...
  inset: 0;
  border-radius: 50%;
  background: radial-gradient(circle at center, #1a1a1a 0%, #2a2a2a 55%, #3a3a3a 100%);
  box-shadow: 
...
.cover-center {
  ...
  border-radius: 50%;
  box-shadow: 0 0 0 3px #1a1a1a, 0 2px 4px rgba(0,0,0,0.5);
  z-index: 3;
}
...
.progress-dot {
  ...
  background: #fff;
  border-radius: 50%;
  box-shadow: 0 0 0 2px #FF6B9D, 0 2px 6px rgba(0,0,0,0.25);
  opacity: 0;
...
.vol-thumb {
  ...
  border-radius: 50%;
  transform: translate(-50%, -50%);
  box-shadow: 0 0 0 2px #FF6B9D, 0 1px 4px rgba(0,0,0,0.2);
...
.pl-item {
  ...
  &.active {
    background: linear-gradient(90deg, rgba(255,154,139,0.18), rgba(255,107,157,0.12));
  }
}
.pl-idx {
  width: 20px;
...
.bar-1 { height: 30%; animation-delay: -0.3s; }
.bar-2 { height: 70%; animation-delay: -0.15s; }
.bar-3 { height: 50%; }
@keyframes playBars {
